...
// NOTE: Waitlist mutations and queries for handling signups
...
// NOTE: Maximum number of lifetime premium spots
...
export const getPremiumStats = query({
  args: {},
  handler: async (ctx) => {
    // NOTE: Query Better Auth user table via component
    // to count users with active premium
...
export const addEmailToWaitlist = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(0, api.waitlist.sendMagicLinkEmail, { email });
